...
def count_in_pack(item_id):
    """Count items of given type in backpack."""
    count = 0
...
def have_in_backpack(item_id, amount=1):
    """Check if backpack has at least X of item."""
    return count_in_pack(item_id) >= amount

def drink_potion(item_id):
    """Try to drink a potion by ID."""
    pot = find_in_backpack_by_id(item_id)
    if pot:
        Items.UseItem(pot)
        return True
    return False
...
def manual_bandage_self():
    ...
    if Journal.Search("You begin applying the bandages"):
        Misc.SendMessage("Bandaging self", 68)
        return True
    elif Journal.Search("You cannot bandage that"):
        Misc.SendMessage("Bandage failed (invalid target)", 33)
        return False
    return True

def manual_healpot():
    if drink_potion(ID_POT_HEAL):
        Misc.SendMessage("Drank heal potion", 68)
    else:
        Misc.SendMessage("No heal potions found!", 33)

def manual_curepot():
    if drink_potion(ID_POT_CURE):
    ...
    if Player.Hits < BANDAGE_HP_THRESHOLD and timer_ready("bandageCD"):
        if have_in_backpack(ID_BANDAGE):
            ...
            if Target.WaitForTarget(500):
                Target.Self()
                reset_timer("bandageCD", 7000)  # ~7 sec bandage delay
...
def step_healpot():
    ...
    if Player.Hits <= HEAL_HP_THRESHOLD and timer_ready("healPotCD"):
        if drink_potion(ID_POT_HEAL):
            Misc.SendMessage("Auto heal pot", 68)
            reset_timer("healPotCD", 10000)

def step_curepot():
    if _auto_cure and Player.Poisoned and timer_ready("curePotCD"):
        if drink_potion(ID_POT_CURE):
            Misc.SendMessage("Auto cure pot", 68)
...
def handle_button(buttonid):
    ...
    elif buttonid == ID_BANDAGE+1000:
        _auto_bandage = not _auto_bandage
        Player.HeadMessage(68 if _auto_bandage else 33,
                           f"Auto Bandage {'ON' if _auto_bandage else 'OFF'}")
    elif buttonid == ID_POT_HEAL+1000:
        _auto_heal = not _auto_heal
        Player.HeadMessage(68 if _auto_heal else 33,
                           f"Auto Heal Pot {'ON' if _auto_heal else 'OFF'}")
    elif buttonid == ID_POT_CURE+1000:
        _auto_cure = not _auto_cure
        Player.HeadMessage(68 if _auto_cure else 33,
                           f"Auto Cure Pot {'ON' if _auto_cure else 'OFF'}")
    elif buttonid == ID_POT_REFRESH+1000:
        _auto_refresh = not _auto_refresh
        Player.HeadMessage(68 if _auto_refresh else 33,
                           f"Auto Refresh Pot {'ON' if _auto_refresh else 'OFF'}")
    elif buttonid == ID_POT_STRENGTH+1000:
        _auto_str = not _auto_str
        Player.HeadMessage(68 if _auto_str else 33,
                           f"Auto STR Pot {'ON' if _auto_str else 'OFF'}")
    elif buttonid == ID_POT_AGILITY+1000:
        _auto_agi = not _auto_agi
        Player.HeadMessage(68 if _auto_agi else 33,
                           f"Auto AGI Pot {'ON' if _auto_agi else 'OFF'}")
    elif buttonid == ID_ORANGE_PETAL+1000:
        _auto_orange = not _auto_orange
        Player.HeadMessage(68 if _auto_orange else 33,
                           f"Auto Orange Petals {'ON' if _auto_orange else 'OFF'}")
    elif buttonid == ID_PURPLE_PETAL+1000:
        _auto_purple = not _auto_purple
        Player.HeadMessage(68 if _auto_purple else 33,
                           f"Auto Purple Petals {'ON' if _auto_purple else 'OFF'}")
