...
CUSTOM_SUMMON_NAMES = {
    ...
    "a water elemental":      "a fira elemental",
    "a air elemental":        "AirElem",
    "a earth elemental":      "Earth",
    "daemon":               "Daemon",
    "a blood elemental":      "BloodElem",
    "a devourer of souls":  "Soul Sucker",
    "a feral mauler":   "Mauler",
    "a volt wisp":  "Pikachu",
    "a poison elemental":   "Siefers Fart",
}
...
def _should_rename(mobile, desired_name):
    global _last_rename
    ...
    current = (mobile.Name or "").strip().lower()
    ...
    now = time.monotonic()
    if serial in _last_rename:
        last_desired, last_time = _last_rename[serial]

        if last_desired.lower() == desired and (now - last_time) < RENAME_FREEZE_SEC:
            return False

    return True


def _rename_summon(mobile, desired_name):
    global _last_rename

    if not _should_rename(mobile, desired_name):
        return

    try:
        Mobiles.Rename(mobile.Serial, desired_name)
        _last_rename[mobile.Serial] = (desired_name, time.monotonic())
        Misc.SendMessage(f"Renamed to: {desired_name}", 68)
    except Exception as e:
        Misc.SendMessage(f"Rename error: {e}", 33)
...
def _try_autotag(m):
    ...
    # HARD STOP: only ever attempt once per serial
    if serial in _rename_done:
        return False
    ...
    try:
        ...
        # Whether we can rename or not, we do not keep retrying forever
        if not desired:
...
def auto_kill_step():
    ...
    if do_all_kill_on_serial(m.Serial):
        ...
        Misc.SendMessage("Auto Kill: %s" % (m.Name or "target"),68)
...
def toggle_auto_kill():
    ...
    Misc.SendMessage("Summon Suite Auto Kill %s (%d tiles)." % (state,AUTO_KILL_RANGE),68 if _auto_kill_enabled else 53)
...
def render_main_page(gd,rows,summon_block_h,list_block_h,total_w,total_h):
    ...
    Gumps.AddLabel(gd,x+20,y,68 if _auto_kill_enabled else 53,"Auto-Kill:ON" if _auto_kill_enabled else "Auto-Kill:OFF")
    y+=22
